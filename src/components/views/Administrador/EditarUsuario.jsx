    ...
    const onSubmit = (usuarioEditado) => {
     ...
     consultaEditarUsuario(usuarioEditado, id).then((respuesta)=>{
      if(respuesta && respuesta.status === 200){
        Swal.fire('Usuario editado', `El usuario ${usuarioEditado.nombreUsuario} fue editado correctamente`, 'success');
        navegacion('/administradorproductos/usuarios');
      }else{
        Swal.fire('Ocurrio un error', `El usuario ${usuarioEditado.nombreUsuario} no fue editado, intentelo mas tarde`, 'error');
      }
     })
    };
